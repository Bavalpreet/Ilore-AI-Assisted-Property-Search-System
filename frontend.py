...
def main():
    ...
    if 'pc' not in st.session_state:
        st.session_state.pc = Pinecone(api_key='<pinecone key>')
    ...
    if st.button("Find Property"):
        ...
        if missing_fields and st.session_state.count:
            ...
            index = st.session_state.pc.Index("iloreai")
            result = []
            temp = []
            results = index.query(vector=embeddings,top_k=3,include_metadata=True)

            for itr2 in results['matches']:
              if itr2['score'] > 0.85:
                description = next(iter(itr2['metadata'].values())).replace("dict_values(['", "").replace("'])", "").strip('"')
